...
window.onload=function(){
	if(isWeiXin() == false){
		var bs={
			versions:function(){
				...
				return {//移动终端浏览器版本信息
					...
					webApp: u.indexOf('Safari') == -1 //是否web应应用程序，没有头部与底部
				};
			}(),
		...
		if(bs.versions.mobile){
			//alert("是手机端");
			//检测手机端浏览器版本
			if(bs.versions.trident==true){
				//alert("IE内核");
			}
			if(bs.versions.presto==true){
				//alert("opera内核");
			}
			if(bs.versions.webKit==true){
				//alert("苹果或者谷歌内核");
			}
			if(bs.versions.gecko==true){
				//alert("火狐内核");
			}
			if(bs.versions.ios==true){
				//alert("ios终端");
			}
			if(bs.versions.android==true){
				//alert("android终端");
			}
			if(bs.versions.iPhone==true){
				//alert("iPhone终端");
			}
			if(bs.versions.iPad==true){
				//alert("iPad");
			}
		}else{
			alert("pc端");
